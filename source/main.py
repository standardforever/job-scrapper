...
from api.v1.routes import scraper_router
from schemas.api_schemas import HealthResponse, ResourceInfo
from service.resource_manager_service import resource_manager
from service.task_manager_service import init_task_manager, get_task_manager
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # Cleanup any stale batches from previous runs
        await task_manager.cleanup_stale_batches(max_age_hours=24)
        logger.info("Stale batches cleaned up")
    ...
    try:
        ...
        logger.info(f"MongoDB connected. Jobs collection has {stats['total_jobs']} documents")
        mongo.close()
    except Exception as e:
        logger.error(f"MongoDB connection failed: {e}")
        raise
    
    logger.info(f"Job Scraper API v{VERSION} started successfully")
    ...
    try:
        from service.batch_executor_service import stop_current_batch
        await stop_current_batch()
    except Exception as e:
        logger.warning(f"Error stopping batch during shutdown: {e}")
    ...
    try:
        ...
        logger.warning(f"Error closing task manager: {e}")
    
    logger.info("Job Scraper API shutdown complete")
...
app = FastAPI(
    ...
    description="""
    API for managing distributed job scraping operations.
    
    ## Features
    
    - **Batch Scraping**: Submit multiple URLs for parallel scraping
    - **Resource Management**: Automatic worker allocation based on server resources
    - **Progress Tracking**: Real-time progress monitoring
    - **Job Storage**: MongoDB-based job storage with querying
    
    ## Endpoints
    
    - `/scrape/batch` - Start batch scraping
    - `/scrape/single` - Test single URL
    - `/scrape/progress` - Get scraping progress
    - `/scrape/stop` - Stop current batch
    - `/scrape/jobs` - List scraped jobs
    """,
    version=VERSION,
    lifespan=lifespan
)
...
async def health_check():
    ...
    # Get resource info
    resource_info = resource_manager.get_resource_info_dict()
    
    # Calculate uptime
    uptime = resource_manager.get_uptime_seconds()
    
    return HealthResponse(
        status="healthy" if mongodb_connected else "degraded",
        version=VERSION,
        mongodb_connected=mongodb_connected,
        resource_info=ResourceInfo(**resource_info),
